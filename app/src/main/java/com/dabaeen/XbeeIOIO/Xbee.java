...
public class Xbee {
    ...
    public String configure(XbeeConfiguration xbeeConfig, OnDataReceivedListener onDataReceivedListener) throws IOException{
        ...
        boolean keepOpen = xbeeConfig.keepOpen;
        ...
        if(!keepOpen) {
             openConfiguration(onDataReceivedListener);
        }
        ...
        if(!keepOpen) {
            closeConfiguration(onDataReceivedListener);
        }
    ...
    public void openConfiguration(OnDataReceivedListener onDataReceivedListener) throws IOException{
        int plusRetry = 3;
        String readBuffer = "";
        ...
        do {
            ...
            if (readBuffer.length() > 0 && onDataReceivedListener != null)
                onDataReceivedListener.onDataReceived(readBuffer);
        }
        while (!readBuffer.contains("OK"));

    }

    public void closeConfiguration(OnDataReceivedListener onDataReceivedListener) throws IOException{
    ...
    /** Sends data to the Xbee device. End with \n, \r, or a combination to send directly. Otherwise
     * you need to flush.
     *
    ...
    public void writeLine(String data) throws IOException{
        ...
        if(data!=null) {
            ...
            if(LogStreams) Log.v(TAG, "Sent: " + data);
    ...
    /** Reads a line of data. Blocks the thread until data is received.
     * A line is ended by \r, \n or \r\n.
     *
     * @return The data read. Null if no data was read.
     * @throws IOException
     */
    public String readLine() throws IOException{
        ...
        while ((line = InReader.readLine()) != null) {
            if(LogStreams && line != null) Log.v(TAG, "Received: " + line);
            return line;
        }
     ...
     */
    public void setOnDataReceivedListener(final OnDataReceivedListener onDataReceivedListener){

        ListenToDataReceived = true;

        if(onDataReceivedListener == null) {
            removeOnDataReceiverListener();
        ...
        ThreadData = new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                while(ListenToDataReceived){
                    if(!isConfiguring){
                        if(checkBuffer()){
                            try {
                                dataReceived(read(), onDataReceivedListener);
                            } catch (IOException e) {
